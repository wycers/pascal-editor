  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/scenes', {
        ...
        body: JSON.stringify({ name: 'Untitled scene', projectId: 'demo', graph: EMPTY_GRAPH }),
      })
      if (!response.ok) {
      ...
      router.push(`/scene/${meta.id}?demo=1`)
    } catch (err) {
  ...
  const handleSaveAs = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/scenes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newName, projectId: 'demo', graph }),
      })
      if (!response.ok) {
